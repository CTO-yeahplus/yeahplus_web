...
// 원본 src/App.jsx 의 LangCtx 이식 — 한/영 토글 + 공용 상수.
import { createContext, useContext, useEffect, useState } from 'react';

export type Lang = 'ko' | 'en';

export const NAV = {
  ko: { home: '홈', privacy: '개인정보', support: '지원', terms: '이용약관' },
  en: { home: 'Home', privacy: 'Privacy', support: 'Support', terms: 'Terms' },
} as const;
...
export function LangProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      const saved = localStorage.getItem('pf_site_lang');
  ...
  const setLang = (l: Lang) => {
    setLangState(l);
    try {
      localStorage.setItem('pf_site_lang', l);
    } catch {}
  };
