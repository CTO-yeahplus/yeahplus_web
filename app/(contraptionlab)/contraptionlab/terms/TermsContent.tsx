...
/* 원본 약관 문서가 없어 sillok·steelstorm 약관을 뼈대로 이 앱에 맞게 고쳤다.
   달라지는 곳은 레벨 에디터와 공유 코드(3조·5조), 그리고 연령(8조)이다.
   사실이 아닌 문장은 쓰지 않는다 — 가격·출시일은 적지 않고, 기능 서술은
   docs/METADATA.md · docs/REVIEW_NOTES.md · README.md 를 따랐다. */
  ...
  return (
    <div className="cp-wrap cp-doc">
      <div className="cp-doc-head">
        ...
        <div className="cp-rev">시행일 2026년 9월 17일 · Effective 17 September 2026</div>
      </div>

      <div className="cp-callout">
        <p>
          이 약관은 주식회사 예아플러스(yeahplus)가 제공하는 iOS 앱 「뚝딱 실험실」의 이용 조건을
          정합니다. 앱을 내려받아 사용하시면 이 약관에 동의하신 것으로 봅니다.
        </p>
        <p style={{ marginTop: '10px' }}>
          These terms govern use of the iOS app &ldquo;Contraption Lab&rdquo; provided by yeahplus.
      ...
      <section>
        ...
        <p>
          ...
          부여됩니다. 가정에서의 사용과{' '}
          <strong>학교 수업 및 학급 활동에서의 사용을 허락합니다.</strong>
        </p>
        <p>다음 행위는 허락되지 않습니다.</p>
        <ul className="cp-bullets">
          <li>앱 또는 앱에 담긴 레벨·그림·음향·코드를 복제하여 재배포하거나 판매하는 행위</li>
          <li>앱을 역컴파일·역설계하거나 보호 수단을 우회하는 행위</li>
          <li>앱의 레벨 데이터를 추출해 별도의 저작물이나 서비스로 재구성하는 행위</li>
        ...
        <div className="cp-en-block">
          ...
          <p>
            ...
            non-commercial purposes. Use at home and{' '}
            <strong>use in school lessons and classroom activities are permitted.</strong> Not
            permitted: redistributing or selling the app or its levels, artwork, sound or code;
            decompiling, reverse engineering or circumventing protections; extracting level data to
            build another work or service; and renting or transferring the app or any part of it for
            a fee.
          </p>
        </div>
      </section>
      ...
      <section>
        ...
        <p>
          ...
          구매와 환불은 App Store를 통해 이루어지고 그 정책을 따릅니다. 예아플러스는 결제 정보를
          전달받지 않으며 직접 환불을 처리할 수 없습니다.
        </p>
        <div className="cp-en-block">
          <h3>2. Purchase and refunds</h3>
          <p>
            This is a paid app with no in-app purchases and no subscription; one purchase unlocks
            everything. Purchases and refunds go through the App Store and follow its policies.
            yeahplus does not receive payment information and cannot process refunds directly.
          </p>
        </div>
      </section>

      <section>
        <h2>3. 계정 없음 · 저장 데이터 · 만든 판의 공유</h2>
        <p>
          이 앱에는 계정과 회원가입이 없습니다. 진행 상황과 설정은{' '}
          <strong>이용자 기기 안에만</strong> 저장되며, 예아플러스는 이를 보관하지 않고 접근할 수도
          없습니다. 따라서{' '}
          <strong>
            앱 삭제, 기기 초기화, 기기 분실 등으로 진행 상황이 사라진 경우 복구가 불가능합니다.
          </strong>{' '}
          기기 사이 이어하기도 지원하지 않습니다.
        </p>
        <p>
          「내 실험실」에서 만든 판은 짧은 글자 코드로 주고받습니다. 이 코드는{' '}
          <strong>서버를 거치지 않고</strong> 이용자가 직접 전달하는 것이며, 예아플러스는 코드를
          보관하거나 열람하지 않습니다. 코드를 어디에 공유할지는 이용자가 정하며, 그에 따른 책임도
          이용자에게 있습니다. 자세한 내용은{' '}
          <Link href="/contraptionlab/privacy">개인정보처리방침</Link>을 봐 주십시오.
        </p>
        <div className="cp-en-block">
          <h3>3. No account, stored data and sharing your own labs</h3>
          <p>
            The app has no account and no sign-up. Progress and settings are stored{' '}
            <strong>only on the user&rsquo;s device</strong>; yeahplus neither holds nor can access
            them. Consequently,{' '}
            <strong>
              progress lost through deletion of the app, a device reset or a lost device cannot be
              recovered,
            </strong>{' '}
            and progress cannot be carried between devices. Labs built in the editor travel as a
            short text code that <strong>never passes through a server</strong> — the user passes it
            on directly, and yeahplus neither stores nor reads it. Where that code is shared, and any
            consequence of sharing it, is the user&rsquo;s own decision. See the{' '}
      ...
      <section>
        <h2>4. 지식재산권</h2>
        <p>
          앱에 담긴 레벨 설계, 개념 설명, 그림, 음향, 코드 등 모든 창작물의 저작권은 주식회사
          예아플러스(yeahplus)에 있습니다. 앱은 제3자의 콘텐츠를 포함하지 않으며, 물리 엔진을 포함해
          모든 코드를 직접 작성했습니다.
        </p>
        <p>
          이용자가 「내 실험실」에서 만든 판의 배치는 만든 사람의 것입니다. 예아플러스는 그것을
          보관하지 않으므로 어떤 권리도 주장하지 않습니다.
        </p>
        <div className="cp-en-block">
          <h3>4. Intellectual property</h3>
          <p>
            Copyright in the app&rsquo;s level designs, concept explanations, artwork, sound and code
            belongs to yeahplus. The app contains no third-party content; all of it, including the
            physics engine, is original. Layouts you create in the editor are yours — we never hold
            them and claim no rights in them.
          </p>
        </div>
      </section>
      ...
      <section>
        ...
        <p>
          이 앱은 물리 개념을 소재로 한{' '}
          <strong>퍼즐 게임이자 학습 보조물</strong>입니다. 교과서나 학술 자료를 대신하지 않으며,
          특정 시험의 성적을 보장하지 않습니다. 게임 안의 물리 계산은 놀이에 필요한 범위에서 단순화한
          모형이며, 현실의 물리 현상을 정밀하게 재현하지 않습니다.
        </p>
        <div className="cp-en-block">
          <h3>5. Nature of the content</h3>
          <p>
            This is a <strong>puzzle game and a learning aid</strong> built on physical ideas. It
            does not replace a textbook or scholarly source and guarantees no examination result.
            The physics inside is a model simplified for play, not a precise reproduction of the
            real world.
          </p>
        </div>
      </section>
      ...
      <section>
        ...
        <p>
          ...
          에 표시된 등급을 따릅니다. 미성년자가 이 앱을 구매하는 경우 보호자의 확인 아래
          이루어지기를 권합니다.
        </p>
        <div className="cp-en-block">
      ...
      <section>
        ...
        <p>
          현재 버전 시행일: <strong>2026년 9월 17일</strong>
        </p>
        <div className="cp-en-block">
